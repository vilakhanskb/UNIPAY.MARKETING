(() => {
  ...
  root.innerHTML = `
    ...
    <section class="ui-section"><div class="container"><div class="section-heading reveal"><div><p class="eyebrow"><span></span>Real product UI</p><h2>UI ຈາກລະບົບຈິງ</h2></div><p>ພາບເຫຼົ່ານີ້ບັນທຶກຈາກ Development Portal ຕົວຈິງ; ຕົວເລກ ແລະສະຖານະຕ່າງໆເປັນຂໍ້ມູນໃນເວລາທີ່ບັນທຶກພາບ.</p></div><div class="ui-gallery ${data.screenshots.length === 1 ? 'single' : ''}">${list(data.screenshots, (shot, index) => `<figure class="ui-shot reveal delay-${index}"><a href="${imagePath(shot.src)}" target="_blank" rel="noreferrer"><img src="${imagePath(shot.src)}" alt="UI ຈິງ: ${shot.title}" width="1440" height="900" loading="lazy" /></a><figcaption><div><span>UI 0${index + 1}</span><h3>${shot.title}</h3></div><p>${shot.caption}</p></figcaption></figure>`)}</div></div></section>
    ...
    <section class="module-cta"><div class="container"><div class="cta-card reveal"><div><p class="eyebrow light"><span></span>${data.label}</p><h2>${data.ctaTitle}</h2><p>ເບິ່ງໂມດູນອື່ນໆ ເພື່ອຮູ້ວ່າຂໍ້ມູນ ແລະຂັ້ນຕອນການເຮັດວຽກເຊື່ອມຕໍ່ກັນແນວໃດ.</p></div><a class="button button-light" href="../../#modules">ເບິ່ງທຸກໂມດູນ <span>→</span></a></div></div></section>`;
